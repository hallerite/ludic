# Shoutouts to Will Brown (@willccbb) for letting me steal his vllm server
...
```bash
uv run python src/ludic_envs/inference/vllm_server.py --model <model> --port <port>
```
...
async def async_send_and_recv(conn: MPConnection, payload: dict, timeout: float = 30.0):
    """Send a payload over the Pipe and await the reply, with handshake-skip and error surfacing."""
    loop = asyncio.get_running_loop()

    async with asyncio.timeout(timeout):
        try:
            # serialize pipe ops
            await loop.run_in_executor(None, pipe_lock.acquire)
            try:
                ...
                while loop.time() - start < timeout:
                    if await loop.run_in_executor(None, conn.poll, 0.1):
                        ...
                        # ---------- NEW ----------
                        # 1) skip the worker’s boot handshake
                        if isinstance(result, dict) and result.get("status") == "ready":
                            continue
                        # 2) propagate worker exceptions cleanly
                ...
                raise asyncio.TimeoutError(f"Worker did not respond within {timeout} s")
            finally:
                pipe_lock.release()
        except Exception:
            logger.exception("async_send_and_recv failed")
            raise
...
def llm_worker(
    ...
    llm = LLM(
        ...
        worker_extension_cls="ludic_envs.inference.vllm_server.WeightSyncWorkerExtension",
    )
                    ...
                    if is_chat_pool:
                        ...
                        if first_chunk_inputs:
                            ...
                            if not active_first_states:
                                ...
                                flags = dict(add_generation_prompt=True)
                                payload = {
                                    "type": "call",
                                    "method": "chat",
                                    "kwargs": {
                            ...
                            if not active_continue_states:
                                ...
                                flags = dict(add_generation_prompt=False)
                                payload = {
                                    "type": "call",
                                    "method": "chat",
                                    "kwargs": {
...
def main(script_args: ScriptArguments):
    ...
    async def lifespan(app: FastAPI):
        ...
        # 🔧 1. Initialise the queue *before* the first request arrives
        request_queue = asyncio.Queue()

        # 🔧 2. Kick off the batch-processing coroutine
        batch_processor_task = asyncio.create_task(
            batch_processing_loop(
                script_args,          # your ScriptArguments
                connections,          # list[MPConnection] to the workers
                request_queue,        # the queue we just made
                logger                # same logger you already use
            )
        )
        ...
        try:
            ...
            for p in processes:      # terminate the vLLM worker procs
                p.terminate()
                p.join()
